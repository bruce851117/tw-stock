...
【摘要格式要求】
你的工作是「忠實摘要」，不是分析、不是評論、不是預測。summary 必須符合以下規則：

- 只能濃縮原文「實際寫出來」的事實與說法，絕對不可以加入原文沒有的任何內容。
- 嚴禁無中生有：不可以加入原文沒有的解讀、影響、評價、推論、產業定位、營運動能、受益或不利等描述。
  【反例】原文若只是「【營收快訊】聯詠(3034)公告6月營收」這種快訊，summary 就只能寫它公告了 6 月營收（有數字就寫數字），
  絕對不可以自行補上「顯示其在相關產業鏈中維持一定的營運成長動能」這類原文根本沒說的話。
- 只有當原文自己有寫分析或因果邏輯時，才照原文把那個邏輯摘要出來；原文沒有，就不要自己生一個出來。
- 開頭先寫「股票名稱（代號）：」（若原文可判斷）。
- 若文章涉及多檔股票，請選出原文中最核心的 2~3 檔股票來摘要。
- 摘要目標 80～100 個中文字，內容不足可以更短，不要硬湊字數，最多不可超過 110 個中文字。
- 摘要必須是完整句子，必須以句號結尾。
- 不可以用「...」或「……」結尾。
- 不可以寫到一半中斷。
